<?php
  include "../../menu/menu.php";
    ...
    if (!array_key_exists('msg_alert', $_GET)) {
      ...
      if ($_GET['msg_alert'] == "success_form"){
        $_GET['sucess'] = "true";
        $_GET['msg'] = "Inventário inserido/adicionado com sucesso!";
      }
      else
      if ($_GET['msg_alert'] == "error_form") {
        $_GET['sucess'] = "false";
        $_GET['msg'] = "Erro ao inserir/cadastrar inventário!";
      }
      else
      if ($_GET['msg_alert'] == "success_excluir") {
        $_GET['sucess'] = "true";
        $_GET['msg'] = "Inventário excluído com sucesso!";
      }
      else
      if ($_GET['msg_alert'] == "error_excluir") {
        $_GET['sucess'] = "false";
        $_GET['msg'] = "Erro ao excluir inventário!";
      }
    }
    /*------------------------------*/
    ...
    $url_produto_gaveta_form = "cadastro_produto_gaveta_form/cadastro_produto_gaveta_form.php";
  /*------------------------------*/

    /*-----------NOVO------------*/
    if (array_key_exists('novo', $_POST)) {
      echo '<script>window.location.href ="' . $url_raiz . $url_produto_gaveta_form . '";</script>';
    }
    /*--------------------------*/
    ...
    if ( array_key_exists('filtrar', $_POST)) {
      ...
      $_POST['produto'] = array_key_exists('produto', $_POST) ? $_POST['produto'] : null;
      $_POST['gaveta'] = array_key_exists('gaveta', $_POST) ? $_POST['gaveta'] : null;
        $url = $url_back . $controller . "FiltrarProdutoGaveta";
        $postData = array(
          'id' => (int)$_POST['id'],
        ...
        $response = file_get_contents("https://localhost:44331/api/Produto_Gaveta/GetProdutoGaveta");
  ...
  if ($_POST) {
    ...
    if ( array_key_exists('edit', $_POST) && $_POST['edit'] != "") {
      echo '<script>window.location.href ="' . $url_raiz . $url_produto_gaveta_form . '?id=' . $_POST['edit'] .'";</script>';
    }
    /*------------------------------*/
  
    /*-----------EXCLUIR------------*/
    if ( array_key_exists('delete', $_POST) && $_POST['delete'] != "") {
      $url = "https://localhost:44331/api/Produto_Gaveta/DeleteProdutoGaveta/" . $_POST['delete'];
      $options = array(
        'http' => array(
          'method'  => 'DELETE'
          )
      ...
      if($response == "201" || $response == "200") {
        echo '<script>window.location.href = "' . $url_raiz . $url_produto_gaveta_list .'?msg_alert=success_excluir";</script>';
      } else {
        echo '<script>window.location.href = "' . $url_raiz . $url_produto_gaveta_list .'?msg_alert=error_excluir";</script>';
      }
    }
    /*------------------------------*/
  }
...
<head>
  ...
  <link rel="stylesheet" href="./cadastro_produto_gaveta_list.scss">
  <title>Document</title>
</head>
<body>
  <h3>Cadastro de inventário</h3>
  <div class="espacamento">
    ...
    <div style="display: flex;">
      ...
      <div class="input" style="width: 30px; margin-left: 150px;">
        <input class="input" name="produto" placeholder="Produto..." type="text">
      </div>
      <div class="input" style="width: 30px; margin-left: 150px;">
        <input class="input" name="gaveta" placeholder="Gaveta..." type="text">
      </div>
    </div>
    <div class="buttons" style="display: flex;">
      ...
      <?php foreach ($_GET['response'] as $item): ?>
        <tr>
          ...
          <td style="width: 30%;"><?= $item -> produto -> codigo . " - " . $item -> produto -> nome ?></td>
          <td style="width: 30%;"><?= $item -> gaveta -> codigo . " - " . $item -> gaveta -> nome ?></td>
      ...
      <?php if ($_GET['sucess'] == "true") : ?>
        <div class="success">
            ...
            <div class="success__close">
              <a href="http://localhost/cadastro_produto_gaveta/cadastro_produto_gaveta_list/cadastro_produto_gaveta_list.php?sucess=ok">
