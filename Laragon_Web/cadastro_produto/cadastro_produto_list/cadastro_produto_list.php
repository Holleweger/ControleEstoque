<?php
  include "../../menu/menu.php";
    ...
    if (!array_key_exists('msg_alert', $_GET)) {
      ...
      if ($_GET['msg_alert'] == "success_form"){
        $_GET['sucess'] = "true";
        $_GET['msg'] = "Produto inserido/adicionado com sucesso!";
      }
      else
      if ($_GET['msg_alert'] == "error_form") {
        $_GET['sucess'] = "false";
        $_GET['msg'] = "Erro ao inserir/cadastrar produto!";
      }
      else
      if ($_GET['msg_alert'] == "success_excluir") {
        $_GET['sucess'] = "true";
        $_GET['msg'] = "Produto excluído com sucesso!";
      }
      else
      if ($_GET['msg_alert'] == "error_excluir") {
        $_GET['sucess'] = "false";
        $_GET['msg'] = "Erro ao excluir produto!";
      }
    }
    /*------------------------------*/
    ...
    $url_produto_form = "cadastro_produto_form/cadastro_produto_form.php";
  /*------------------------------*/

    /*-----------NOVO------------*/
    if (array_key_exists('novo', $_POST)) {
      echo '<script>window.location.href ="' . $url_raiz . $url_produto_form . '";</script>';
    }
    /*--------------------------*/
    ...
    if ( array_key_exists('filtrar', $_POST)) {
      ...
      $_POST['codigo'] = array_key_exists('codigo', $_POST) ? $_POST['codigo'] : null;
        $url = $url_back . $controller . "FiltrarProduto";
        $postData = array(
          'id' => (int)$_POST['id'],
          'nome' => $_POST['nome'],
          'codigo' => $_POST['codigo'],
        ...
        $response = file_get_contents("https://localhost:44331/api/Produto/GetProduto");
  ...
  if ($_POST) {
    ...
    if ( array_key_exists('edit', $_POST) && $_POST['edit'] != "") {
      echo '<script>window.location.href ="' . $url_raiz . $url_produto_form . '?id=' . $_POST['edit'] .'";</script>';
    }
    /*------------------------------*/
  
    /*-----------EXCLUIR------------*/
    if ( array_key_exists('delete', $_POST) && $_POST['delete'] != "") {
      $url = "https://localhost:44331/api/Produto/DeleteProduto/" . $_POST['delete'];
      $options = array(
        'http' => array(
          'method'  => 'DELETE'
          )
      ...
      if($response == "201" || $response == "200") {
        echo '<script>window.location.href = "' . $url_raiz . $url_produto_list .'?msg_alert=success_excluir";</script>';
      } else {
        echo '<script>window.location.href = "' . $url_raiz . $url_produto_list .'?msg_alert=error_excluir";</script>';
      }
    }
    /*------------------------------*/
  }
...
<head>
  ...
  <link rel="stylesheet" href="./cadastro_produto_list.scss">
      ...
      <?php if ($_GET['sucess'] == "true") : ?>
        <div class="success">
            ...
            <div class="success__close">
              <a href="http://localhost/cadastro_produto/cadastro_produto_list/cadastro_produto_list.php?sucess=ok">
